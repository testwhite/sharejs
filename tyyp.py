...
for i in range(len(ty_username)):
    ...
    def main():
        ...
        if ("errorCode" in response.text):
            print(response.text)
            res2 = f"第一次抽奖: {response.json().get('errorCode')}"
        else:
            description = response.json()['description']
            if description in ["1", 1]:
               description = "50M 空间"
            print(f"第一次抽奖获得{description}")
            res2 = f"第一次抽奖获得{description}"
        response = s.get(url2, headers=headers)
        if ("errorCode" in response.text):
            print(response.text)
            res3 = f"第二次抽奖: {response.json().get('errorCode')}"
        else:
            description = response.json()['description']
            if description in ["1", 1]:
               description = "50M 空间"
            print(f"第二次抽奖获得{description}")
            res3 = f"第二次抽奖获得{description}"

        response = s.get(url3, headers=headers)
        if ("errorCode" in response.text):
            print(response.text)
            res4 = f"第三次抽奖: {response.json().get('errorCode')}"
        else:
            description = response.json()['description']
            if description in ["1", 1]:
               description = "50M 空间"
            print(f"第三次抽奖获得{description}")
            res4 = f"第三次抽奖获得{description}"
        message = res1+res2+res3+res4
        Push(contents=message)
        notify.send("天翼云盘",res1 + '\n' + res2 + '\n' + res3 + '\n' + res4)
