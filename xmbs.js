...
const step = randomFriendPin($.getdata('xmMinStep')*1 || 18000, $.getdata('xmMaxStep')*1 || 20150);
function getToken() {
  if ($response.body) {
  ...
  if (login_token) {
    ...
    if ($.tokenInfo && $.tokenInfo.result === 'ok') {
      ...
      if ($.changeStepRes && $.changeStepRes.code === 1) {
		...
		if ($.isNode()) {
          await notify.sendNotify(`小米运动`,`步数修改成功:${step}步,时间：${timeFormat(localtime())}‍`);
        }
      ...
      $.msg($.name, '失败', `Token已失效，请重新获取`)
		if ($.isNode()) {
          await notify.sendNotify(`小米运动`,`步数修改失败:Token已失效，请重新获取`);
        }	  
    }
  } else {
    $.log('暂无Token')
